...
<div class="page-title">
    <h2>Configurar PagSeguro Cartão de Cŕedito</h2>
</div>
<div class="content mar-bottom-30">
    <div class="container-fluid pad-bottom-30">
        <?= $this->Form->create($pagseguro_credit_card, ['type' => 'file']) ?>
        <div class="row">
            <div class="col-md-4 form-group">
                <?= $this->Form->control('label', ['class' => 'form-control', 'label' => 'Nome para exibição', 'value' => $pagseguro_credit_card->label]) ?>
            </div>

            <div class="col-md-4 form-group">
                <?= $this->Form->control('sandbox_email', ['class' => 'form-control', 'label' => 'E-mail de compra para testes Sandbox', 'value' => $pagseguro_credit_card->sandbox_email]) ?>
            </div>
        </div>
        <div class="row">
            <div class="col-md-4">
                <div class="form-group">
                    <?= $this->Form->control('email', ['class' => 'form-control', 'label' => ['text' => 'E-mail <span class="glyphicon glyphicon-question-sign" aria-hidden="true" data-toggle="tooltip" data-placement="top" title="E-mail da conta do PagSeguro."></span>', 'escape' => false], 'value' => $pagseguro_credit_card->email]) ?>
                </div>
            </div>

            <div class="col-md-4">
                <div class="form-group">
                    <?= $this->Form->control('token', ['class' => 'form-control', 'label' => ['text' => 'Token <span class="glyphicon glyphicon-question-sign" aria-hidden="true" data-toggle="tooltip" data-placement="top" title="O Token de Segurança pode ser encontrado no portal do vendedor do PagSeguro: https://pagseguro.uol.com.br > minha conta > preferência > integrações > utilização de apis > gerar token."></span>', 'escape' => false], 'value' => $pagseguro_credit_card->token]) ?>
                </div>
            </div>
            ...
            <div class="col-md-2">
                <div class="form-group">
                    ...
                    <?= $this->Form->control('environment', ['label' => false, 'empty' => 0, 'options' => $environments, 'type' => 'radio', 'value' => $pagseguro_credit_card->environment]) ?>
                </div>
            </div>
            ...
            <div class="col-md-2">
                <div class="form-group">
                    <label>Status</label>
                    <?= $this->Form->control('status', ['label' => false, 'empty' => 0, 'options' => $statuses, 'type' => 'radio', 'value' => $pagseguro_credit_card->status]) ?>
                </div>
            </div>
        </div>
        <hr>
