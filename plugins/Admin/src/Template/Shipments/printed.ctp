 ...
 */
$this->Html->script(['shipments/printed.functions.js'], ['fullBase' => true, 'block' => 'scriptBottom']);
?>
<div class="page-title">
    <h2>Configurar Impresso</h2>
</div>
<div class="content mar-bottom-30">
    <div class="container-fluid pad-bottom-30">
        <?= $this->Form->create($printed, ['id' => 'form-interval']) ?>
        <div class="row">
            <div class="col-md-4">
                <div class="form-group">
                    <label>Status</label>
                    <?= $this->Form->control('status', ['label' => false, 'empty' => 0, 'options' => $statuses, 'type' => 'radio', 'value' => $printed->status]) ?>
                </div>
            </div>
        </div>
        <hr>

        <h3>Faixas de CEPS que recebem encomendas pelo metodo Impresso</h3>
        <p>
        ...
        <div class="row table-responsive" id="list-interval">
            <div class="col-md-12">
                <table class="table table-hover">
                    ...
                    <tbody>
                    <?php if ($printed->interval): ?>
                        <?php
                        $count = 0;
                        foreach ($printed->interval as $key => $interval): ?>
                            <tr>
